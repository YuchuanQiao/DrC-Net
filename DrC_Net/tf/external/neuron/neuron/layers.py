...
class SpatialDiff(Layer):
    """
    N-D Spatial difference of Tensorflow / Keras Layer

    The Layer get the difference of two inputs.
    """
...
class SpatialDiffWithMask(Layer):
    """
    N-D Spatial difference of Tensorflow / Keras Layer

    The Layer get the difference of two inputs with a mask.
    """
    ...
    def build(self, input_shape):
        """
        input_shape should be a list for three inputs:
        input1: image.
        input2: image.
        input3: mask.
        """
    ...
    def call(self, inputs):
        """
        Parameters
            inputs: list with three entries
        """

        # check shapes
        assert len(inputs) == 3, "inputs has to be len 3, found: %d" % len(inputs)
        vol1 = inputs[0]
        vol2 = inputs[1]
        vol3 = tf.dtypes.cast(inputs[2], tf.float32)

        vol_diff = (vol1 - vol2)

        vol = tf.math.multiply(vol_diff,vol3)

        return vol

class SpatialMask(Layer):
    """
    N-D Spatial average of Tensorflow / Keras Layer

    The Layer get the mask from two inputs.
    """
    ...
    def call(self, inputs):
        ...
        vol = (vol1 + vol2)
        vol = tf.where(vol>0,1,0)

        return vol


class SpatialAvg(Layer):
    """
    N-D Spatial average of Tensorflow / Keras Layer

    The Layer get the average of two inputs.
    """
...
class SpatialTransformer(Layer):
    ...
    def _single_transform(self, inputs):
        return transform(inputs[0], inputs[1], interp_method=self.interp_method,phase_encoding=self.phase_encoding)
